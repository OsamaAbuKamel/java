...
/**
 * An example of how to implement your own layout of controls contained inside a
 * Pane. The JavaFX Pane class does not
 * do any layout of its children. The Pane shows the children where the children
 * wants to be layed out themselves.
 * Thus, by changing layout position of the children of a Pane you can change
 * it's layout.
 *
 * Future potential improvements:
 * - Remodel the code so it more clearly shows the algorithm - even though it
 * may slow the algorithm down a bit.
...
public class AutoResponsiveLayout {
    ...
    public static class PaneLayoutInfo {
        ...
        public int determineRowCountFromWidgetLayoutInfos() {
            this.noOfRows = getLastWidgetLayoutInfo(this.widgetLayoutInfos).rowNo + 1; // row numbers are 0-based (first
                                                                                       // row has index 0)
        ...
        private double calcMinimumUsedRowHeights() {
            ...
            for (int rowNo = 0; rowNo < this.noOfRows; rowNo++) {
                ...
                while (rowEndIndex < this.widgetLayoutInfos.size()
                        && rowNo == this.widgetLayoutInfos.get(rowEndIndex).rowNo) {
    ...
    // Feature flags. All features should be enabled for a fully automatic
    // responsive layout
    ...
    public AutoResponsiveLayout(Pane pane, ScrollPane parent) {
        ...
        this.targetPane.widthProperty()
                .addListener((ObservableValue<? extends Number> property, Number oldValue, Number newValue) -> {
                    this.paneLayoutInfo.maxRowWidth = newValue.doubleValue();
                    layoutPane();
                });

        parent.viewportBoundsProperty()
                .addListener((ObservableValue<? extends Bounds> property, Bounds oldValue, Bounds newValue) -> {
    ...
    protected void layoutPane() {
        ...
        // Phase 2.1: Now that number of rows is decided, calculate the average width of
        // rows
        paneLayoutInfo.calculateTotalChildMinWidth();
        paneLayoutInfo.calculateAverageRowWidthBasedOnMinWidths();

        // Phase 2.2: Assign children to rows based on average row width instead of max
        // row width - for a more even distribution of children.
        if (this.balanceRows) {
            assignChildrenToRowsUsingAverageRowWidth();
        }

        // Phase 2.3 Pull up "dangling" widgets towards the top of the pane, so the grid
        // looks more similar.
    ...
    private int determineRowCountFromWidgetLayoutInfos() {
        return getLastWidgetLayoutInfo(this.paneLayoutInfo.widgetLayoutInfos).rowNo + 1; // row numbers are 0-based
    ...
    private void expandWidgetWidthsToFitRow(double newWidth, ObservableList<Node> children) {
        ...
        for (int rowNo = 0; rowNo < paneLayoutInfo.noOfRows; rowNo++) {
            ...
            while (rowEndIndex < this.paneLayoutInfo.widgetLayoutInfos.size()
                    && rowNo == this.paneLayoutInfo.widgetLayoutInfos.get(rowEndIndex).rowNo) {
                rowEndIndex++;
            }
            ...
            // System.out.println(" Max Row Width : " + newWidth + "(" +
            // paneLayoutInfo.maxRowWidth + ")");
    ...
    private void expandRowHeights() {
        ...
        if (this.paneLayoutInfo.visibleHeight > minimumUsedRowHeights) {
            // unusedHeight = Math.max(0, this.targetPane.getHeight() -
            // minimumUsedRowHeights);
            unusedHeight = Math.max(0, this.paneLayoutInfo.visibleHeight - minimumUsedRowHeights);
        }

        // if minimumUsedRowHeights is larger than available height - do NOT use a
        // negative unusedHeight - but use 0.
